# add pright_config to pyproject.toml
pr-init() {
    local pyright_config
    read -rd '' pyright_config << 'EOF'
...
EOF
    ...
    if [ -f "pyproject.toml" ] ; then
        if grep -q "\[tool.pyright\]" "pyproject.toml" ; then
            echo "[tool.pyright] already exists in pyproject.toml"
            return 1
        else
            echo "Adding [tool.pyright] settings to pyproject.toml ..."
            printf "%s\n" "$pyright_config" >> pyproject.toml
            return 0
        fi
    else
        echo "pyproject.toml does not exist"
        echo -e "Use '\033[1;33muv init\033[0m' to initialize the project first"
        return 1
    fi
}

# uv command recommandation
_uv() {
